...
async def get_user_benefit(id: str):
    return user_benefits_serializer(
        user_benefit_collection.find_one({"_id": ObjectId(id)})
    )
...
async def create_user_benefit(benefit: UserBenefit):
    ...
    return user_benefits_serializer(
        user_benefit_collection.find({"_id": _id.inserted_id})
    )
...
async def update_user_benefit(id: str, benefit: UserBenefit):
    user_benefit_collection.find_one_and_update(
